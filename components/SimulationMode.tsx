...
const SimulationMode: React.FC<SimulationModeProps> = ({ prompt, onComplete, onCancel }) => {
  ...
  useEffect(() => {
    const startVideoGeneration = async () => {
      try {
        operationRef.current = await aiOrchestrator.generateVideo(prompt);
      ...
      <div className="holographic-panel w-full max-w-4xl h-[90vh] flex flex-col p-6 border-2 border-primary-t-50 shadow-2xl shadow-primary/20">
        <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center gap-3">
                ...
                <button onClick={handleLogToChat} disabled={!videoUrl} className="px-4 py-2 rounded-md bg-primary-t-80 text-background hover:bg-primary flex-shrink-0 disabled:opacity-50 transition-all duration-200 transform hover:scale-105 active:scale-100">
                    Log to Chat
                </button>
            </div>
        </div>
        ...
        <div className="flex-1 bg-slate-900/50 border border-primary-t-20 rounded-lg flex items-center justify-center overflow-hidden">
            {isLoading && (
                <div className="flex flex-col items-center justify-center h-full text-slate-400 text-center">
                   <svg className="w-12 h-12 animate-spin text-primary mb-4" xmlns="http://www.w.org/2000/svg" fill="none" viewBox="0 0 24 24">
