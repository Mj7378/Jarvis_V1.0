...
export const saveFaceProfile = (imageDataUrl: string): void => {
  const profile = getProfile();
  profile.faceDataUrl = imageDataUrl;
  saveProfile(profile);
};

export const getFaceProfile = (): string | null => {
  return getProfile().faceDataUrl || null;
};

export const deleteFaceProfile = (): void => {
  const profile = getProfile();
  delete profile.faceDataUrl;
  saveProfile(profile);
};
